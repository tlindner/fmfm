...
byte x_DCOPC;  /* DSKCON OPERATION CODE 0-3 */
byte x_DCDRV;  /* DSKCON DRIVE NUMBER 0—3 */
byte x_DCTRK;  /* DSKCON TRACK NUMBER 0—34 */
byte x_DCSEC;  /* DSKCON SECTOR NUMBER 1-18 */
byte *x_DCBPT; /* DSKCON DATA POINTER */
...
byte x_DRGRAM;    /* RAM IMAGE OF DSKREG ($FF40) */
byte x_DR0TRK[4];    /* CURRENT TRACK NUMBER, DRIVES 0,1,2,3 */
byte x_NMIFLG;    /* NMI FLAG: 0=DON'T VECTOR <>0=YECTOR OUT */
void *x_DNMIVC;   /* NMI VECTOR: WHERE TO JUMP FOLLOWING AN NMI */
...
unsigned long x_dskcon_init(x_dskcon_NmiServiceFunctionPointer newNMIService)
{
    ...
    *isr = 0x7E;  // JMP
    * (x_dskcon_NmiServiceFunctionPointer *) (isr + 1) = newNMIService;
...
asm __norts__ void x_dskcon_processSector()
{
    /* CAUTION:
